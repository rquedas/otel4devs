...
Now, let's test one of `tracemock` settings validation rules. Remove the `number_of_traces` setting from the `config.yaml`, and here is what the output for running the collector will look like:
